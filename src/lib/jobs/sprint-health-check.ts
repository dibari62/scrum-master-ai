...
export async function checkOrganizationHealth(
  ...
  for (const projectRow of projectRows) {
    ...
    const sprintRows = await scope.reads.sprintsByProject(project.id);
     ...
     */
    const verdict: HealthVerdict = result.available
    ...
    outcomes.push({ projectId: project.id, sprintId: running.id, verdict });
  }
