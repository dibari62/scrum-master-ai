...
| `ForecastMethod` | Metodo di previsione | Come si è ottenuta la velocity stimata: `yesterdays-weather` \| `focus-factor` \| `default-focus-factor`. Dichiarato sempre insieme al numero. | Una previsione che cambia metodo da uno sprint all'altro cambia significato senza dirlo, e nessuno può contestarla perché nessuno sa cosa afferma. |
| `forecastVariance` | Scostamento dalla previsione | Velocity effettiva meno velocity prevista, in punti, col segno. | Una differenza e non un rapporto: un rapporto nasconde la dimensione dello sprint, e sbagliare di tre punti su cinque non è come sbagliarne tre su cinquanta. |
