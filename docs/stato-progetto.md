...
| **Groq non è collegato al suo SDK** | [ADR-0005](architecture/ADR-0005-provider-llm.md), `src/lib/llm/gateway.ts` | ~~anche Gemini~~ **Gemini è collegato** (`src/lib/llm/google.ts`, via REST e senza dipendenze nuove). Groq resta un segnaposto che si dichiara non configurato, e la schermata delle impostazioni **lo dice**: tacerlo manderebbe qualcuno a generare una credenziale per scoprire poi che non succede nulla |
